'use client'

import React from 'react'

const Footer = () => {
  return (
    <footer className="w-full bg-[#fdf6f0] text-black py-12 px-8 md:px-16">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Main Content */}
          <div className="lg:col-span-2">
            <h3 className="font-serif font-bold text-2xl mb-4 cursor-pointer hover:text-[#005bbb] transition-colors">
              Forge your future
            </h3>
            <p className="text-gray-700 mb-4">
              Building the next generation of student entrepreneurs and innovators.
            </p>
            <a 
              href="mailto:contactubforge@gmail.com" 
              className="text-[#005bbb] hover:underline font-medium"
            >
              contactubforge@gmail.com
            </a>
          </div>

          {/* Social Media Links */}
          <div>
            <h4 className="font-semibold text-lg mb-4">Connect</h4>
            <div className="flex space-x-4">
              <a 
                href="https://www.linkedin.com/company/forgeatub" 
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-[#005bbb] rounded-full flex items-center justify-center hover:bg-[#004499] transition-colors"
                aria-label="LinkedIn"
              >
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                </svg>
              </a>
              <a 
                href="https://x.com/ubforge?s=21" 
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-[#005bbb] rounded-full flex items-center justify-center hover:bg-[#004499] transition-colors"
                aria-label="Twitter"
              >
                <svg className="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                </svg>
              </a>
              <a 
                href="https://www.instagram.com/ub.forge"
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-[#005bbb] rounded-full flex items-center justify-center hover:bg-[#004499] transition-colors"
                aria-label="Instagram"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="text-white"
                >
                  <rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect>
                  <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
                  <line x1="17.5" y1="6.5" x2="17.51" y2="6.5"></line>
                </svg>
              </a>
            </div>
          </div>

          {/* Events Button */}
          <div>
            <h4 className="font-semibold text-lg mb-4">Get Involved</h4>
            <a 
              href="https://luma.com/calendar/manage/cal-zJkt69wozQWZhcK"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-block bg-[#005bbb] text-white px-6 py-3 rounded-lg hover:bg-[#004499] transition-colors font-medium"
            >
              View Events
            </a>
          </div>
        </div>

        {/* Copyright */}
        <div className="border-t border-gray-300 mt-8 pt-8 text-center text-gray-600">
          <p>© 2025 UB Forge. All rights reserved.</p>
        </div>
      </div>
    </footer>
  )
}

export default Footer 